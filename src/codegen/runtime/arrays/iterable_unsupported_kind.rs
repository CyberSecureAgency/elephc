...
/// only hash tables are supported; indexed arrays inside iterable parameters are
/// rejected at runtime because element-type metadata is not yet propagated through
...
pub fn emit_iterable_unsupported_kind(emitter: &mut Emitter) {
    ...
    emitter.instruction("mov x2, #54");                                         // pass the fixed iterable runtime fatal message length to write()
...
fn emit_iterable_unsupported_kind_linux_x86_64(emitter: &mut Emitter) {
    ...
    emitter.instruction("mov edx, 54");                                         // pass the fixed iterable runtime fatal message length to write()
