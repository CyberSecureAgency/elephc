...
pub fn emit_http_build_request(emitter: &mut Emitter) {
    ...
    //   [sp, 240] saved x29
    //   [sp, 248] saved x30
    emitter.instruction("sub sp, sp, #256");
    emitter.instruction("stp x29, x30, [sp, #240]");
    ...
    emitter.instruction("add x4, sp, #104");
    emitter.instruction("add x5, sp, #112");
    emitter.instruction("bl __rt_get_string_context_option");
    ...
    emitter.instruction("add x4, sp, #120");
    emitter.instruction("add x5, sp, #128");
    emitter.instruction("bl __rt_get_string_context_option");
    ...
    let lookup_str = |emitter: &mut Emitter, sym: &str, len: i64, ptr_off: i64| {
        emitter.instruction(&format!("str xzr, [sp, #{}]", ptr_off));
        emitter.instruction(&format!("str xzr, [sp, #{}]", ptr_off + 8));
        abi::emit_symbol_address(emitter, "x0", "_http_key_str");
        emitter.instruction("mov x1, #4");
        abi::emit_symbol_address(emitter, "x2", sym);
        emitter.instruction(&format!("mov x3, #{}", len));
        emitter.instruction(&format!("add x4, sp, #{}", ptr_off));
        emitter.instruction(&format!("add x5, sp, #{}", ptr_off + 8));
        emitter.instruction("bl __rt_get_string_context_option");
    };
    ...
    emitter.instruction("cbz x11, __rt_hbr_skip_ie_aarch64");
    ...
    emitter.instruction("b.eq __rt_hbr_skip_ie_aarch64");
    ...
    emitter.instruction("cbz x11, __rt_hbr_skip_mr_aarch64");
    emitter.instruction("ldr x12, [sp, #200]");                                 // follow_location_ptr
    emitter.instruction("ldrb w13, [x12]");
    emitter.instruction("cmp w13, #48");                                        // '0' = falsy
    emitter.instruction("b.eq __rt_hbr_skip_mr_aarch64");
    ...
    emitter.instruction("cbz x11, __rt_hbr_mr_store_aarch64");
    ...
    emitter.instruction("cmp x16, x11");
    emitter.instruction("b.ge __rt_hbr_mr_store_aarch64");
    emitter.instruction("ldrb w13, [x12, x16]");
    emitter.instruction("sub w13, w13, #48");                                   // '0'..'9' → 0..9
    emitter.instruction("cmp w13, #9");
    emitter.instruction("b.hi __rt_hbr_mr_store_aarch64");                      // non-digit terminates
    emitter.instruction("mov x17, #10");
    emitter.instruction("mul x15, x15, x17");
    ...
    emitter.instruction("b __rt_hbr_mr_loop_aarch64");
    emitter.label("__rt_hbr_mr_store_aarch64");
    emitter.instruction("str x15, [x10]");
    ...
    emitter.instruction("cbz x11, __rt_hbr_skip_to_aarch64");
    emitter.instruction("ldr x12, [sp, #152]");                                 // timeout_ptr
    emitter.instruction("mov x15, #0");
    emitter.instruction("mov x16, #0");
    emitter.label("__rt_hbr_to_loop_aarch64");
    emitter.instruction("cmp x16, x11");
    emitter.instruction("b.ge __rt_hbr_to_store_aarch64");
    emitter.instruction("ldrb w13, [x12, x16]");
    emitter.instruction("sub w13, w13, #48");
    emitter.instruction("cmp w13, #9");
    emitter.instruction("b.hi __rt_hbr_to_store_aarch64");
    emitter.instruction("mov x17, #10");
    emitter.instruction("mul x15, x15, x17");
    emitter.instruction("add x15, x15, x13");
    emitter.instruction("add x16, x16, #1");
    emitter.instruction("b __rt_hbr_to_loop_aarch64");
    ...
    emitter.instruction("b.eq __rt_hbr_no_fulluri_aarch64");
    ...
    emitter.instruction("bl __rt_http_build_copy_aarch64");
    ...
    emitter.instruction("ldr x11, [sp, #24]");
    emitter.instruction("bl __rt_http_build_copy_aarch64");
    emitter.instruction("str x9, [sp, #48]");
    ...
    emitter.instruction("cbz x12, __rt_hbr_no_header_aarch64");
    ...
    emitter.instruction("cbz x12, __rt_hbr_no_clen_aarch64");
    ...
    emitter.instruction("mov x11, #21");
    emitter.instruction("bl __rt_http_build_copy_aarch64");
    emitter.instruction("str x9, [sp, #48]");

    // -- if content was found, append the body bytes after the blank line --
    emitter.instruction("ldr x12, [sp, #96]");
    emitter.instruction("cbz x12, __rt_hbr_no_body_aarch64");
    ...
    emitter.instruction("ldp x29, x30, [sp, #240]");
    emitter.instruction("add sp, sp, #256");
    ...
    emitter.instruction("b.ge __rt_http_build_copy_done_aarch64");
    ...
    emitter.instruction("b __rt_http_build_copy_loop_aarch64");
...
fn emit_http_build_request_linux_x86_64(emitter: &mut Emitter) {
    ...
    // Expose host to __rt_http_open's follow_location loop.
    emitter.instruction("mov QWORD PTR [rip + _http_active_host_ptr], rdi");
    emitter.instruction("mov QWORD PTR [rip + _http_active_host_len], rsi");

    // Preload method = default ("GET", 3)
    emitter.instruction("lea r9, [rip + _http_default_method]");
    emitter.instruction("mov QWORD PTR [rbp - 40], r9");
    emitter.instruction("mov r9, 3");
    ...
    emitter.instruction("lea rdi, [rip + _http_key_str]");
    emitter.instruction("mov rsi, 4");                                          // strlen("http")
    emitter.instruction("lea rdx, [rip + _http_method_key_str]");
    ...
    emitter.instruction("lea rdx, [rip + _http_header_key_str]");
    ...
    emitter.instruction("mov QWORD PTR [rbp - 88], 0");
    emitter.instruction("mov QWORD PTR [rbp - 96], 0");
    emitter.instruction("lea rdi, [rip + _http_key_str]");
    emitter.instruction("mov rsi, 4");
    emitter.instruction("lea rdx, [rip + _http_content_key_str]");
    emitter.instruction("mov rcx, 7");                                          // strlen("content")
    emitter.instruction("lea r8, [rbp - 88]");
    emitter.instruction("lea r9, [rbp - 96]");
    emitter.instruction("call __rt_get_string_context_option");
    emitter.instruction("mov QWORD PTR [rbp - 104], rax");                      // content_found flag

    // Context lookup: [http][user_agent]
    emitter.instruction("mov QWORD PTR [rbp - 112], 0");
    emitter.instruction("mov QWORD PTR [rbp - 120], 0");
    emitter.instruction("lea rdi, [rip + _http_key_str]");
    emitter.instruction("mov rsi, 4");
    emitter.instruction("lea rdx, [rip + _http_user_agent_key_str]");
    emitter.instruction("mov rcx, 10");                                         // strlen("user_agent")
    emitter.instruction("lea r8, [rbp - 112]");
    emitter.instruction("lea r9, [rbp - 120]");
    emitter.instruction("call __rt_get_string_context_option");

    // Context lookup: [http][protocol_version]
    emitter.instruction("mov QWORD PTR [rbp - 128], 0");
    emitter.instruction("mov QWORD PTR [rbp - 136], 0");
    emitter.instruction("lea rdi, [rip + _http_key_str]");
    emitter.instruction("mov rsi, 4");
    emitter.instruction("lea rdx, [rip + _http_protocol_version_key_str]");
    emitter.instruction("mov rcx, 16");                                         // strlen("protocol_version")
    emitter.instruction("lea r8, [rbp - 128]");
    emitter.instruction("lea r9, [rbp - 136]");
    emitter.instruction("call __rt_get_string_context_option");
    ...
    let lookup_str_x = |emitter: &mut Emitter, sym: &str, len: i64, ptr_off: i64| {
        emitter.instruction(&format!("mov QWORD PTR [rbp - {}], 0", ptr_off));
        emitter.instruction(&format!("mov QWORD PTR [rbp - {}], 0", ptr_off + 8));
        emitter.instruction("lea rdi, [rip + _http_key_str]");
        emitter.instruction("mov rsi, 4");
        emitter.instruction(&format!("lea rdx, [rip + {}]", sym));
        emitter.instruction(&format!("mov rcx, {}", len));
        emitter.instruction(&format!("lea r8, [rbp - {}]", ptr_off));
        emitter.instruction(&format!("lea r9, [rbp - {}]", ptr_off + 8));
        emitter.instruction("call __rt_get_string_context_option");
    };
    ...
    // ignore_errors: truthy when ptr non-zero AND first byte != '0'.
    emitter.instruction("lea r10, [rip + _http_active_ignore_errors]");
    ...
    emitter.instruction("jz __rt_hbr_skip_ie_x");
    emitter.instruction("mov r12, QWORD PTR [rbp - 176]");                      // ignore_errors_ptr
    emitter.instruction("movzx eax, BYTE PTR [r12]");
    emitter.instruction("cmp al, 48");                                          // '0' falsy
    emitter.instruction("je __rt_hbr_skip_ie_x");
    ...
    emitter.instruction("cmp rcx, r11");
    emitter.instruction("jge __rt_hbr_mr_store_x");
    emitter.instruction("movzx eax, BYTE PTR [r12 + rcx]");
    emitter.instruction("sub al, 48");                                          // '0'..'9' → 0..9
    emitter.instruction("cmp al, 9");
    emitter.instruction("ja __rt_hbr_mr_store_x");                              // non-digit terminates
    emitter.instruction("imul r15, r15, 10");
    emitter.instruction("movzx rax, al");
    emitter.instruction("add r15, rax");
    emitter.instruction("inc rcx");
    emitter.instruction("jmp __rt_hbr_mr_loop_x");
    emitter.label("__rt_hbr_mr_store_x");
    emitter.instruction("mov QWORD PTR [r10], r15");
    emitter.label("__rt_hbr_skip_mr_x");
    // timeout: parse seconds as base-10 int.
    emitter.instruction("lea r10, [rip + _http_active_timeout_seconds]");
    emitter.instruction("mov QWORD PTR [r10], 0");
    emitter.instruction("mov r11, QWORD PTR [rbp - 168]");                      // timeout_len
    emitter.instruction("test r11, r11");
    emitter.instruction("jz __rt_hbr_skip_to_x");
    emitter.instruction("mov r12, QWORD PTR [rbp - 160]");                      // timeout_ptr
    emitter.instruction("xor r15, r15");
    emitter.instruction("xor rcx, rcx");
    emitter.label("__rt_hbr_to_loop_x");
    emitter.instruction("cmp rcx, r11");
    emitter.instruction("jge __rt_hbr_to_store_x");
    emitter.instruction("movzx eax, BYTE PTR [r12 + rcx]");
    emitter.instruction("sub al, 48");
    emitter.instruction("cmp al, 9");
    emitter.instruction("ja __rt_hbr_to_store_x");
    emitter.instruction("imul r15, r15, 10");
    emitter.instruction("movzx rax, al");
    emitter.instruction("add r15, rax");
    emitter.instruction("inc rcx");
    emitter.instruction("jmp __rt_hbr_to_loop_x");
    ...
    emitter.instruction("mov QWORD PTR [rip + _http_active_proxy_ptr], r11");
    emitter.instruction("mov r11, QWORD PTR [rbp - 200]");                      // proxy_len
    emitter.instruction("mov QWORD PTR [rip + _http_active_proxy_len], r11");
    ...
    emitter.instruction("call __rt_http_build_copy_x86");
    emitter.instruction("mov QWORD PTR [rbp - 56], rax");

    // write ' ' separator
    emitter.instruction("mov rdi, QWORD PTR [rbp - 56]");
    emitter.instruction("mov BYTE PTR [rdi], 32");
    ...
    emitter.instruction("movzx eax, BYTE PTR [r11]");
    emitter.instruction("cmp al, 48");                                          // '0' (falsy)
    emitter.instruction("je __rt_hbr_no_fulluri_x");
    emitter.label("__rt_hbr_emit_fulluri_x");
    emitter.instruction("mov rdi, QWORD PTR [rbp - 56]");
    emitter.instruction("lea rsi, [rip + _http_scheme_prefix]");
    ...
    emitter.instruction("jne __rt_hbr_proto_default_x");
    emitter.instruction("mov r11, QWORD PTR [rbp - 128]");                      // proto_version_ptr
    emitter.instruction("movzx eax, BYTE PTR [r11]");
    emitter.instruction("cmp al, 49");                                          // '1'
    emitter.instruction("jne __rt_hbr_proto_default_x");
    emitter.instruction("movzx eax, BYTE PTR [r11 + 1]");
    emitter.instruction("cmp al, 46");                                          // '.'
    emitter.instruction("jne __rt_hbr_proto_default_x");
    emitter.instruction("movzx eax, BYTE PTR [r11 + 2]");
    emitter.instruction("cmp al, 49");                                          // '1'
    emitter.instruction("jne __rt_hbr_proto_default_x");
    emitter.instruction("lea rsi, [rip + _http_version_host_11]");
    emitter.label("__rt_hbr_proto_default_x");
    emitter.instruction("mov rdx, 17");
    emitter.instruction("call __rt_http_build_copy_x86");
    emitter.instruction("mov QWORD PTR [rbp - 56], rax");
    ...
    // copy "\r\n" after the Host: value
    emitter.instruction("mov rdi, QWORD PTR [rbp - 56]");
    emitter.instruction("lea rsi, [rip + _http_crlf]");
    emitter.instruction("mov rdx, 2");
    emitter.instruction("call __rt_http_build_copy_x86");
    emitter.instruction("mov QWORD PTR [rbp - 56], rax");
    ...
    emitter.instruction("test r10, r10");
    emitter.instruction("jz __rt_hbr_no_ua_x");
    emitter.instruction("mov rdi, QWORD PTR [rbp - 56]");
    emitter.instruction("lea rsi, [rip + _http_user_agent_prefix]");
    ...
    emitter.instruction("test r10, r10");
    emitter.instruction("jz __rt_hbr_no_header_x86");
    emitter.instruction("mov rdi, QWORD PTR [rbp - 56]");
    emitter.instruction("mov rsi, QWORD PTR [rbp - 64]");
    ...
    emitter.instruction("test r10, r10");
    emitter.instruction("jz __rt_hbr_no_clen_x86");
    emitter.instruction("mov rdi, QWORD PTR [rbp - 56]");
    emitter.instruction("lea rsi, [rip + _http_content_length_prefix]");
    ...
    // copy "Connection: close\r\n\r\n"
    emitter.instruction("mov rdi, QWORD PTR [rbp - 56]");
    emitter.instruction("lea rsi, [rip + _http_trailer]");
    emitter.instruction("mov rdx, 21");
    emitter.instruction("call __rt_http_build_copy_x86");
    emitter.instruction("mov QWORD PTR [rbp - 56], rax");

    // if content was found, append body bytes after the blank line
    emitter.instruction("mov r10, QWORD PTR [rbp - 104]");
    emitter.instruction("test r10, r10");
    emitter.instruction("jz __rt_hbr_no_body_x86");
    emitter.instruction("mov rdi, QWORD PTR [rbp - 56]");
    emitter.instruction("mov rsi, QWORD PTR [rbp - 88]");
    emitter.instruction("mov rdx, QWORD PTR [rbp - 96]");
    emitter.instruction("call __rt_http_build_copy_x86");
    emitter.instruction("mov QWORD PTR [rbp - 56], rax");
    ...
    emitter.instruction("lea r9, [rip + _http_req_scratch]");
    emitter.instruction("sub rax, r9");
    ...
    emitter.instruction("jge __rt_http_build_copy_done_x86");
    emitter.instruction("mov r10b, BYTE PTR [rsi + rcx]");
    emitter.instruction("mov BYTE PTR [rdi + rcx], r10b");
    emitter.instruction("add rcx, 1");
    emitter.instruction("jmp __rt_http_build_copy_loop_x86");
    emitter.label("__rt_http_build_copy_done_x86");
    emitter.instruction("mov rax, rdi");
    emitter.instruction("add rax, rdx");
    emitter.instruction("ret");
}
