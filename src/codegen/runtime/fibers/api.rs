...
pub fn emit_fiber_throw_state_error(emitter: &mut Emitter) {
    if emitter.target.arch == Arch::X86_64 {
        emit_x86_64_stub(emitter, "__rt_fiber_throw_state_error");
        return;
    }
...
pub fn emit_fiber_construct(emitter: &mut Emitter) {
    if emitter.target.arch == Arch::X86_64 {
        emit_x86_64_stub(emitter, "__rt_fiber_construct");
        return;
    }
    ...
    emitter.instruction(&format!("sub x10, x1, #{}", FIBER_INITIAL_STACK_FRAME_BYTES)); // x10 = initial saved_sp (room for the switch save area)
    ...
    emitter.instruction(&format!("mov x12, #{}", FIBER_INITIAL_STACK_FRAME_BYTES / 16)); // number of 16-byte chunks to zero
    ...
    emitter.instruction(&format!("str x9, [x10, #{}]", FIBER_INITIAL_FRAME_X29_X30_OFFSET + 8)); // saved x30 slot = entry trampoline address
...
pub fn emit_fiber_start(emitter: &mut Emitter) {
    if emitter.target.arch == Arch::X86_64 {
        emit_x86_64_stub(emitter, "__rt_fiber_start");
        return;
    }
...
pub fn emit_fiber_resume(emitter: &mut Emitter) {
    if emitter.target.arch == Arch::X86_64 {
        emit_x86_64_stub(emitter, "__rt_fiber_resume");
        return;
    }
...
pub fn emit_fiber_suspend(emitter: &mut Emitter) {
    if emitter.target.arch == Arch::X86_64 {
        emit_x86_64_stub(emitter, "__rt_fiber_suspend");
        return;
    }
...
pub fn emit_fiber_throw(emitter: &mut Emitter) {
    if emitter.target.arch == Arch::X86_64 {
        emit_x86_64_stub(emitter, "__rt_fiber_throw");
        return;
    }
...
pub fn emit_fiber_get_current(emitter: &mut Emitter) {
    if emitter.target.arch == Arch::X86_64 {
        emit_x86_64_stub(emitter, "__rt_fiber_get_current");
        return;
    }
    emitter.blank();
...
pub fn emit_fiber_get_return(emitter: &mut Emitter) {
    if emitter.target.arch == Arch::X86_64 {
        emit_x86_64_stub(emitter, "__rt_fiber_get_return");
        return;
    }
    emitter.blank();
...
pub fn emit_fiber_state_getter(emitter: &mut Emitter) {
    if emitter.target.arch == Arch::X86_64 {
        emit_x86_64_stub(emitter, "__rt_fiber_state_eq");
        return;
    }
    emitter.blank();
