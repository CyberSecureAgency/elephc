...
/// Total bytes saved on the stack by a context switch (must stay 16-aligned).
const SWITCH_SAVE_BYTES: i32 = 160;
...
pub fn emit_fiber_switch(emitter: &mut Emitter) {
    ...
    emitter.instruction(&format!("sub sp, sp, #{}", SWITCH_SAVE_BYTES));        // reserve space for the full switch save area
    ...
    emitter.instruction(&format!("add sp, sp, #{}", SWITCH_SAVE_BYTES));        // release the switch save area on the target's stack
...
pub(crate) const FIBER_INITIAL_STACK_FRAME_BYTES: i32 = SWITCH_SAVE_BYTES;

/// Offset within the initial frame where x29/x30 are stored (entry trampoline address goes into x30).
pub(crate) const FIBER_INITIAL_FRAME_X29_X30_OFFSET: i32 = 80;

fn emit_x86_64_stub(emitter: &mut Emitter) {
    emitter.blank();
    emitter.comment("--- runtime: fiber_switch (x86_64 stub) ---");
    emitter.label_global("__rt_fiber_switch");
