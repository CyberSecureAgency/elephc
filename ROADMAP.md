...
- [x] Phase 41 — `stream_get_contents($stream, ?$length, $offset = -1)`: the optional `$length` (max bytes) and `$offset` (seek-before-read) arguments are honored. A finite `$length` routes through the wrapper-aware `__rt_fread`; `$offset >= 0` seeks first (lseek for a normal fd, the wrapper's `stream_seek` for a synthetic fd); a `null`/negative `$length` reads to EOF. The read-all/wrapper-drain path is factored into `emit_read_all_from_fd`. Limitation: on a non-seekable stream a finite `$length` is a single bounded read, so it may return fewer bytes than requested in one call. ARM64 + x86_64 (both Docker-verified)
- [x] Phase 42 — `stream_copy_to_stream($from, $to, ?$length, $offset = -1)`: the optional `$length`/`$offset` are honored via a single capped, feof-gated `__rt_fread`/`__rt_fwrite` loop that works for any real/wrapper fd combination (seeks `$from` by `$offset >= 0`, stops at `$length` bytes copied or source EOF; `null`/negative `$length` copies to EOF). The no-extra-args fast path (real fds → `__rt_stream_copy_to_stream`, wrapper fds → compiled loop) is unchanged. ARM64 + x86_64 (both Docker-verified)
- [x] Phase 43 — `file_get_contents()` over `http://`/`https://`/`ftp://`/`ftps://` URLs: a literal URL opens the matching wrapper (the fd-producing core of each wrapper factored into a shared `emit_open_fd`), slurps the whole body via `__rt_stream_get_contents`, persists it to owned heap with `__rt_str_persist`, and returns it (`false` on a failed open) — the same wrappers as `fopen()`, so `file_get_contents` now covers every URL scheme `fopen` does. A literal `https://`/`ftps://` URL pulls in `-lelephc_tls` via the checker (http/ftp stay libc-only). ARM64 + x86_64 (both Docker-verified)
