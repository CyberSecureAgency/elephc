...
- [x] Filesystem path manipulation: `basename()`, `dirname()`, `pathinfo()` (both the single-flag form returning a string and the no-flag form returning an associative array), `realpath()`, `fnmatch()` with shell-glob `*` / `?` / `[...]` / `\\` support, plus the `PATHINFO_DIRNAME` / `PATHINFO_BASENAME` / `PATHINFO_EXTENSION` / `PATHINFO_FILENAME` / `PATHINFO_ALL` constants
