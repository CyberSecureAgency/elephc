...
# Run tests with the project mounted as a volume. `cargo build -p elephc-sqlite`
# first so the PDO bridge staticlib (libelephc_sqlite.a) exists in the target
...
if [ ${#TEST_ARGS[@]} -eq 0 ]; then
    ...
    docker run \
        ...
        -w /app \
        "$IMAGE" \
        sh -c 'cargo build -p elephc-tls -p elephc-sqlite && cargo test'
else
    ...
    docker run \
        ...
        -w /app \
        "$IMAGE" \
        sh -c 'cargo build -p elephc-tls -p elephc-sqlite && cargo test "$@"' sh "${TEST_ARGS[@]}"
fi
