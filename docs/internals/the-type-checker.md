...
`Iterable` represents PHP's `iterable` pseudo-type (`array | Traversable`). It is treated as a type-erased 8-byte raw heap pointer at runtime — the checker accepts `Array` and `AssocArray` arguments for parameters declared `iterable`, and `foreach` over an `iterable` local types both `$key` and `$value` as `Mixed`. Operations on iterable values (`foreach`, `echo`, `gettype()`, `var_dump()`, `===`, scalar casts, `is_iterable()`) dispatch through the `__rt_heap_kind` runtime helper instead of routing through the boxed `__rt_mixed_*` helpers used by `Mixed`/`Union(_)`. `foreach` over an iterable is only supported when the underlying heap kind is a hash table; indexed-array iterables abort at runtime through `__rt_iterable_unsupported_kind`.
