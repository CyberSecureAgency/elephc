...
| Callable forms are descriptor-backed | `new Fiber(...)` accepts closures, first-class callables, descriptor-valued variables, runtime string callbacks, static-method callable arrays, stored and literal instance-method callable arrays such as `$cb = [$object, "method"]`, and simple invokable-object variables. |
