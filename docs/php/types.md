...
| `is_callable()` | `is_callable($val): bool`    | Returns true for closures, first-class callables, strings naming known builtins or user functions, `[$obj, "method"]` arrays with public methods, and objects with public `__invoke()`. |
