...
| `iterable`       | Yes              | PHP pseudo-type for `array \| Traversable`. Type-erased raw heap pointer; runtime operations (`foreach`, `echo`, `gettype()`, `var_dump()`, `===`, casts, `is_iterable()`) dispatch on the heap-kind tag. |
...
- `iterable` `foreach` is supported only when the underlying value is a hash table at runtime. Indexed-array iterables (`Array(_)`) trigger a fatal runtime error because the indexed-array header does not yet carry an element-type tag, so the loop cannot determine how to read each element. Object-implemented `Traversable` is also not yet supported.
